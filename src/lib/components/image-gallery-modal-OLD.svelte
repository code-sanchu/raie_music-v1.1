...
<Transition show={isOpen}>
	<Dialog class="relative z-[70]" on:close={() => (isOpen = false)}>
		...
		<TransitionChild
			...
			<div class="flex-grow w-[80vw] max-h-[80vh] border border-red-600 overflow-hidden">
				<div
					...
					{#each images as image}
						<div class="w-[80vw] shrink-0 grid place-items-center">
							...
							<div
								class={`cursor-zoom-out flex flex-col max-h-[80vh] max-w-[78vw] border border-blue-400`}
								style:width={`${
								...
								{#if image.caption}
									<Caption
										extraClasses="!mt-0 shrink-0 py-xs pl-xs no-underline !text-my-black bg-white/60 sm:!text-base rounded-b-sm">
										{image.caption}
									</Caption>
								{/if}
							</div>
