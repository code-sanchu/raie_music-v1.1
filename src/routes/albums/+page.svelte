...
<PageLayout.Body>
	...
	{#if mounted}
		<div
			class="prose leading-relaxed italic text-lg tracking-wider text-my-black-700 max-w-[700px]"
			in:fade>
			<p class="">
			...
			<p class="">
				Raie’s prolific writing talent combined with <Link href="https://www.wesonator.com/"
					>Wes Maebe’s</Link> international award-winning engineering and production skills … a host
				of top UK/US players and some Love (that essential ingredient!) thrown in for good measure ….
				equals a recipe for success!
			</p>

			<p>
				Have a listen on the easy clicks below … and you can check out Raie’s <Link
					href={raie_links.listen.youtube.href}>YouTube page</Link> for some video chats and music film
				footage …
			</p>
		</div>
	{/if}
	...
	{#if mounted}
		...
		{/each} -->
					<p>
						Raie worked with <Link href="https://www.highvioletprandplugging.com/"
							>High Violet PR</Link>
						to promote the re-launch of This Music Thing.
					</p>
					<p>
						The album has attracted some notoriety across the globe – featuring on various radio
						stations.
					</p>
				</svelte:fragment>
			</Album>
